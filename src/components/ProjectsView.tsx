...
interface ProjectsViewProps {
  ...
  onFetchAiSummary?: (id: string) => Promise<string>;
}
...
export default function ProjectsView({ 
  ...
  const [editTasksCount, setEditTasksCount] = useState(10);
  const [editProgress, setEditProgress] = useState(0);

  // AI Summary Refresh States
  ...
  const startEditing = () => {
    ...
    setEditTeamSize(selectedProject.teamSize);
    setEditTasksCount(selectedProject.tasksCount);
  ...
  const handleUpdateSubmit = (e: React.FormEvent) => {
    ...
    const updates = {
      ...
      status: editStatus,
      dueDate: editDueDate,
      teamSize: Number(editTeamSize),
      tasksCount: Number(editTasksCount),
      progress: Number(editProgress)
    };

    onUpdateProject(selectedProject.id, updates);
  ...
  const handleRegenerateSummary = async () => {
    if (!selectedProject || !onFetchAiSummary) return;
    setIsRefreshingSummary(true);
    try {
      const summary = await onFetchAiSummary(selectedProject.id);
      setSelectedProject(prev => prev ? { ...prev, aiSummary: summary } : null);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleDelete = () => {
    ...
    if (confirm(`Are you sure you want to delete project "${selectedProject.name}"?`)) {
        ...
        {selectedProject && (
          <>
            ...
            <motion.div 
              ...
              className="fixed top-0 right-0 h-screen w-full sm:w-[480px] bg-white border-l border-slate-200 p-6 shadow-2xl z-50 flex flex-col justify-between overflow-y-auto text-left font-sans"
            >
              {isEditing ? (
                <form onSubmit={handleUpdateSubmit} className="space-y-6 flex flex-col h-full justify-between pb-6">
                  <div className="space-y-5">
                    {/* Header Actions */}
                    <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                      <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Edit Project Details</span>
                      <button 
                        type="button"
                    ...
                    <div className="space-y-1 text-xs">
                      <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Project Name *</label>
                      <input 
                        type="text" 
                        required
                        value={editName}
                    ...
                    {/* Description */}
                    <div className="space-y-1 text-xs">
                      <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Description *</label>
                      <textarea 
                        required
                        rows={3}
                        value={editDesc}
                        onChange={(e) => setEditDesc(e.target.value)}
                        className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:outline-none font-semibold text-slate-800 text-xs resize-none focus:ring-2 focus:ring-indigo-500/10 transition-all"
                      />
                    </div>

                    {/* Category */}
                    <div className="space-y-1 text-xs">
                      <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Category</label>
                      <select 
                        value={editCategory}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {/* Priority */}
                      <div className="space-y-1 text-xs">
                        <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Priority</label>
                        <select 
                          value={editPriority}
                      ...
                      {/* Status */}
                      <div className="space-y-1 text-xs">
                        <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Status</label>
                        <select 
                          value={editStatus}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {/* Due Date */}
                      <div className="space-y-1 text-xs">
                        <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Due Date</label>
                        <input 
                          type="date" 
                          value={editDueDate}
                      ...
                      {/* Progress */}
                      <div className="space-y-1 text-xs">
                        <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Progress (%)</label>
                        <input 
                          type="number" 
                          min={0}
                          max={100}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {/* Team Size */}
                      <div className="space-y-1 text-xs">
                        <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Team Size</label>
                        <input 
                          type="number" 
                          min={1}
                          value={editTeamSize}
                      ...
                      {/* Tasks Count */}
                      <div className="space-y-1 text-xs">
                        <label className="text-slate-500 font-bold block mb-1 uppercase text-[10px] tracking-wider">Total Tasks</label>
                        <input 
                          type="number" 
                          min={0}
                          value={editTasksCount}
                          onChange={(e) => setEditTasksCount(Number(e.target.value))}
                          className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:outline-none font-semibold text-slate-800 text-xs focus:ring-2 focus:ring-indigo-500/10 transition-all"
                        />
                      </div>
                    </div>
                      ...
                      <div className="flex items-center justify-between text-indigo-600">
                        ...
                        {onFetchAiSummary && (
                          <button 
                            ...
                            className="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 flex items-center gap-1 bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1 rounded-lg disabled:opacity-50 transition-colors cursor-pointer"
                          >
                            {isRefreshingSummary ? (
                              <>
                  ...
                  <div className="pt-6 border-t border-slate-100 mt-6 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <button 
                        ...
                        className="px-4 py-2 text-xs font-bold text-indigo-600 hover:text-indigo-700 border border-indigo-200 hover:border-indigo-300 bg-indigo-50/50 hover:bg-indigo-50 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
                      >
                        Edit Details
                      </button>
                      <button 
                        onClick={handleDelete}
                        className="px-4 py-2 text-xs font-bold text-rose-600 hover:text-rose-700 border border-rose-200 hover:border-rose-300 bg-rose-50/30 hover:bg-rose-50 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
                      >
                        Delete Project
                      </button>
                    </div>
