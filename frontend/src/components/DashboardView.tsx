...
import { Project, Task, Document, ChatConversation, Notification } from '../types';
...
export default function DashboardView({
  ...
  // Today's Date
  const formattedDate = "Friday, July 3, 2026";
  ...
  // Today's Priorities - Meetings schedule
  const todaysMeetings = [
    { time: '10:00 AM', title: 'Sprint Kickoff & Team Alignment', host: 'Sarah Jenkins', type: 'Engineering' },
    { time: '1:30 PM', title: 'Security Audit & Server Connection Check', host: 'Elena Rostova', type: 'Security' },
    { time: '4:00 PM', title: 'Q2 Deliverable Finalization Review', host: 'Marcus Vance', type: 'Product' }
  ];

  // AI Suggestion list (diagnostics)
  const aiSuggestions = [
    { title: 'Marcus Vance has high workload', desc: 'Assign "SYN-101: System testing" to Dr. Alex Rivera (currently at 40% workload).', tab: 'teams' },
    { title: 'New meeting transcript uploaded', desc: 'A new meeting transcript was uploaded. Click to generate a summary and tasks.', tab: 'meeting-intel' }
  ];

  // AI input helper chips
  const helperChips = [
  ...
  // Timeline Activity Feed data (Redesign requirement)
  const timelineActivities = [
    { time: '10 mins ago', user: 'Sarah Jenkins', action: 'uploaded Design_Spec_v2.pdf', project: 'Synapse Core' },
    { time: '1 hour ago', user: 'AI Assistant', action: 'generated Kickoff meeting summary', project: 'Meeting Intel' },
    { time: '2 hours ago', user: 'System', action: 'uploaded security guidelines document', project: 'Aegis System' },
    { time: 'Yesterday', user: 'Alex Rivera', action: 'created Project Horizon specs', project: 'Workspace' },
    { time: '2 days ago', user: 'Marcus Vance', action: 'assigned 4 tasks to Applied AI department', project: 'Athena' }
  ];
  ...
  const handleAskWorkspaceAi = (text: string) => {
    if (!text.trim()) return;
    setAiLoading(true);
    setAiResponseText(null);
    setTimeout(() => {
      let resp = "I've checked the workspace for you. You have 3 pending tasks for Project Athena, and Project Alpha is 75% complete. I also recommend Elena Rostova to help review your compliance documents.";
      const t = text.toLowerCase();
      if (t.includes('meeting')) {
        resp = "Here is your meeting summary: Sarah Jenkins will coordinate the project roadmap, and Dr. Alex Rivera is working on server performance. All action items have been added to your tasks.";
      } else if (t.includes('report')) {
        resp = "Here is today's report: All systems are running smoothly. The team workload is well-balanced, and connection security is stable.";
      } else if (t.includes('team') || t.includes('member')) {
        resp = "Here are my recommendations: Dr. Alex Rivera is the best fit for Project Horizon. He has the required skills (Python, Go) and has 40% workload capacity.";
      } else if (t.includes('pending') || t.includes('tasks')) {
        resp = `There are currently ${pendingTasks.length} pending tasks. Of these, ${highPriorityPendingTasks.length} are marked as High Priority.`;
      } else if (t.includes('alpha')) {
        resp = "Project Alpha is currently on track at 75% progress. Team members assigned are Sarah Jenkins and Dr. Alex Rivera. The target completion date is July 8, 2026.";
      }
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          {[
            ...
            { label: 'Meetings', value: 42, sub: 'Summaries ready', icon: Video, tab: 'meeting-intel' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-3 text-center space-y-1">
                  <span className="text-xl font-extrabold text-indigo-600 block">3</span>
                ...
                <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-3 text-center space-y-1">
                  <span className="text-xl font-extrabold text-emerald-600 block">2</span>
                ...
                <div className="bg-rose-50/50 border border-rose-100 rounded-2xl p-3 text-center space-y-1">
                  <span className="text-xl font-extrabold text-rose-500 block">1</span>
          ...
          <div className="premium-card p-6 lg:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { name: 'Sarah Jenkins', role: 'Lead AI Scientist', status: 'Online', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&q=80&w=120' },
                { name: 'Dr. Alex Rivera', role: 'Principal RAG Engineer', status: 'In Meeting', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=120' },
                { name: 'Maya Chen', role: 'Senior Cloud Architect', status: 'Working Remotely', avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=120' },
                { name: 'Marcus Vance', role: 'Staff Full-Stack Engineer', status: 'Busy', avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=120' }
              ].map((emp, i) => {
                ...
                return (
                  <div key={i} className="flex items-center justify-between p-2 rounded-xl bg-slate-50/50 hover:bg-slate-50 border border-slate-100/50 transition-all">
                    <div className="flex items-center gap-3">
                      <div className="relative shrink-0">
                        <img src={emp.avatar} alt="" className="w-8.5 h-8.5 rounded-lg object-cover" />
                        <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border border-white ${colors[emp.status] || 'bg-slate-400'}`} />
                      </div>
