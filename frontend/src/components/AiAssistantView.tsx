...
export default function AiAssistantView({
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      // Connect to live FastAPI chat model proxy
      const response = await api.post(`/chats/${activeChat.id}/messages/ai`, { text });
      if (response.data) {
        const aiMsg = response.data.text || response.data.answer || 'Response processed successfully.';
        const refs = response.data.references || [];
        await onAddMessage(activeChat.id, 'ai', aiMsg, refs);
        setLoading(false);
        return;
      }
    } catch (error) {
      console.warn('Real AI assistant endpoint offline, triggering local simulation.', error);
    }
    ...
    setTimeout(async () => {
      ...
      if (text.toLowerCase().includes('search') || text.toLowerCase().includes('file')) {
