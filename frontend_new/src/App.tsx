...
export default function App() {
  ...
  // Intercept window.alert for safe visual rendering inside browser iframe sandbox
  useEffect(() => {
  ...
  const handleLogin = async (email?: string, password?: string) => {
    try {
      await authService.login(email || 'marcus.vance@synapse.ai', password || '••••••••');
      setIsLoggedIn(true);
      await loadAllData();
    } catch (error) {
  ...
  // Project AI Summary Generation Handler
  const handleFetchAiSummary = async (id: string): Promise<string> => {
    const summary = await projectService.getAiSummary(id);
    return summary;
  };

  // Team Member Creation Handler
  const handleCreateMember = async (newMember: Omit<TeamMember, 'id'>) => {
    const created = await teamService.createMember(newMember);
    await loadAllData();
    showToast(`Team member "${created.name}" added successfully!`, 'success');
  };
  ...
  const handleUpdateMember = async (id: string, updates: Partial<TeamMember>) => {
    ...
    await loadAllData();
    showToast(`Team member "${updated.name}" updated successfully!`, 'success');
  };
  ...
  const handleUploadDocument = async (file: File, name: string, sizeBytes: number, category: string) => {
    const created = await documentService.uploadDocument(file, name, sizeBytes, category);
    await loadAllData();
  ...
  const handleSaveMeetingAsDoc = async (title: string, summary: string, minutes: string) => {
    try {
      const textContent = `Title: ${title}\n\nSummary:\n${summary}\n\nMinutes:\n${minutes}`;
      const blob = new Blob([textContent], { type: 'text/plain' });
      const filename = `${title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_minutes.txt`;
      const file = new File([blob], filename, { type: 'text/plain' });
      
      await documentService.uploadDocument(file, filename, blob.size, 'Meetings');
      await loadAllData();
      showToast(`Meeting minutes saved to Document Library as "${filename}"!`, "success");
      handleAddNotification(
        'Meeting Minutes Saved',
        `"${title}" minutes have been successfully saved and indexed in the Document Library.`,
        'Medium',
        'System'
      );
    } catch (err) {
      console.warn("Failed to save meeting as document:", err);
      showToast("Could not save meeting minutes automatically.", "error");
    }
  };
              ...
              <ViewSuspense>
                ...
                {currentTab === 'documents' && (
                  <DocumentsView 
                    ...
                    onRefreshDocuments={loadAllData}
                  />
                )}
                ...
                {currentTab === 'meeting-intel' && (
                  <MeetingIntelView 
                    ...
                    onSaveMeeting={handleSaveMeetingAsDoc}
                  />
                )}
